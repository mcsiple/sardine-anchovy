...
# NOTE: There will be lots of errors because there are some regions where there is only one time series, or one of the variables isn't present in the dataset (e.g., biomass in the FAO dataset does not exist). If I coded this better it would be cleaner. You just have to 
...
#Easier for troubleshooting if you do one data source at a time
...
d=3
  for(r in 1:length(regions)){
    ...
    for(v in 3:3){ # landings only for FAO
      var.list[[v]] <- getMARSSstates(data = alldat,
                                      region_or_subregion = region,
                                      scale = "Region", 
                                      data_source = data_source,
...
# Depracated but was asked for by a reviewer
# data.list <- list()
# region.list <- list()
# regions <- unique(alldat$region)
# dsources <- unique(alldat$datasource) #Barange, RAM, FAO
