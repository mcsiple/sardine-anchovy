...
test.y <- t(get.mds.ts(length = longyrs,
                       autocorrs = c(0.98,0.67), # estimates from the global B matrix in the initial step
                       rho = 0.5,
                       sds = c(0.34,0.43), #sqrt(c(.12,.19))
                       driver.period = 50,
                       driver.amp = 0.5,
...
obslengths <- seq(30,longyrs,by=10) # values small than 30 lead to errors in wilcoxon tests
length(obslengths)
...
outs$truediff <- rep(true.d$diff,times=length(obslengths)) #length(obslengths)-2
outs$trueloCI <- rep(true.d$CI.L,times=length(obslengths)) #length(obslengths)-2
outs$truehiCI <- rep(true.d$CI.U,times=length(obslengths)) #length(obslengths)-2
#outs$timescale = rep(c('less.than.5','five.ten','ten.plus'),times=length(obslengths))
...
# Experiment: 
# Generate a true time series known to be asynchronous. 
# Sample that time series with different lengths of data (e.g., 10 yrs, 30 yrs)
# Try to detect asynchrony using our method, based on that length of the time series

nsims = 1 # how many times to repeat the experiment
nwmr = 50 # how many surrogate time series to use for null dist
longyrs = 150 # how many years of pop size to generate
...
for(sim in 1:nsims){
  test.yy <- t(get.mds.ts(autocorrs = c(0.98,0.67), # these are estimates from the B matrix in the initial step
                          rho = 0.43,
                          c(0.34,0.43), #sqrt(q1,q2)
                          driver.period = 50,
                          driver.amp = 0.5,
                          CC = matrix(c(1, -1), ncol = 1),length = longyrs))
  
  ts_test <- list(std_sardine=as.numeric(scale(test.yy[,1])),
                  std_anchovy=as.numeric(scale(test.yy[,2])))
  giantnull2 <- get_large_null2(timeseries=ts_test,nsims=nwmr)
...
for(l in 3:length(obslengths)){ #get errors for wilcox tests with short ts and long time periods, so start with longer ones.
  len <- obslengths[l]
  #start.ind <- sample(x = 1:(longyrs-len),size = 1,replace = TRUE ) # randomize
  start.ind <- 1
  end.ind <- start.ind+(len-1)
  #if(end.ind>longyrs){end.ind = longyrs} # if random window is at the end of the ts
  obs <- list(std_anchovy = ts_test$std_anchovy[start.ind:end.ind],
              std_sardine = ts_test$std_sardine[start.ind:end.ind])
  pearson.cor <- cor(obs$std_anchovy,obs$std_sardine,method="pearson")
  spearman.cor <- cor(obs$std_anchovy,obs$std_sardine,method="spearman") # this is the method used in at least 1 other study
  giantnull.obs <- get_large_null2(timeseries=obs,nsims=nwmr)
...
end_time <- Sys.time()
(total_sim_time <- end_time - start_time) #100 sims takes 18 hours (!)

giant_mess <- list(list.outs,list.true.d)


save(giant_mess,file = "GIANT_power_analysis_R1.RData")
...
load("R/PowerAnalysis/GIANT_power_analysis_R1.RData") #data frame giant_mess
#for some reason I saved all the sims in giant_mess[[1]][[50]]
df <- giant_mess[[1]][[50]][-1,]
df$sim <- rep(1:50,each=39) #should be 1:nsims
...
summary$timescale = factor(summary$timescale, levels=c('less.than.5','five.ten','ten.plus'))
levels(summary$timescale) <- c("<5 yr","5-10 yr",">10 yr")
write.csv(summary,file = "TableS5_Power.csv")

fig.s16 <- ggplot(summary,aes(x=obs.length,y=prob.in.CI)) + 
            geom_line(lwd=1.2) + 
            facet_wrap(~timescale) +
            theme_bw(base_size=14) +
            ylab("Prob(estimate of d \n within CI of true value)") +
            xlab("Number of years of observation")

tiff(here::here("R/Figures/FigS16_probInWindow.tiff"),
...
} # There will be errors about binding character and factor vectors but it's ok
...
for(s in 1:nsims){
  ...
  test.typeI <- t(get.mds.ts(length = longyrs,
                             autocorrs = c(0.67,0.67), #FIX THESE
                             rho = 0.06, 
                             sds = sqrt(c(0.12,0.19)),
                             driver.period = 60,
                             driver.amp = 0.5,
                             CC = matrix(c(1,1), ncol = 1)))
ts_test <- list(std_sardine=as.numeric(scale(test.typeI[,1])),
                std_anchovy=as.numeric(scale(test.typeI[,2])))
...
# Plot the probability of a Type 1 error vs. how long our time series are --------
# As tiff:
