...
extract.medians <- function(data = alldat, region_or_subregion, scale = "Region", data_source, variable = "landings"){
  #' @param data = dataset including the time series you're interested in
  #' @param region_or_subregion name of the region or subregion that you want to extract max values for;
  #' either a subregion or 1 of 5 LMEs (Benguela, California, NE Atlantic, Kuroshio-Oyashio, Humboldt)
  #' @param scale = whether the name in "region_or_subregion" is the name of a full LME or a subregion
  ...
  if(variable=="landings"){
    lt.medians <- dataset %>% 
      group_by(sp,stock) %>% 
      summarize(median.var=round(median(landings,na.rm=TRUE),2))}
  
  if(variable=="ssb"){
    lt.medians <- dataset %>% 
      group_by(sp,stock) %>% 
      summarize(median.var=round(median(ssb,na.rm=TRUE),2))}
  
  if(variable=="rec"){
    lt.medians <- dataset %>% 
      group_by(sp,stock) %>% 
      summarize(median.var=round(median(rec,na.rm=TRUE),2))}
  if(variable=="fishing.mortality"){ # Max (F) doesn't make sense, but including it anyway to be thorough
    lt.medians <- dataset %>% 
      group_by(sp,stock) %>% 
      summarize(median.var=round(median(fishing.mortality,na.rm=TRUE),2))}
  
  print(lt.medians)
  
  #anchovy stats
  lt.max.a <- max(lt.medians[which(lt.medians$sp=="Anchovy"),ncol(lt.medians)],na.rm=TRUE) # Look among the median anchovy biomass, landings, or rec-- which species has the highest median value  
  lt.max.sp <- as.character(lt.medians[lt.medians$median.var==lt.max.a,2])    # Which anchovy species had highest median value for this time series (i.e., the "dominant anchovy species")
  
  #sardine stats
  lt.max.s <- max(lt.medians[which(lt.medians$sp=="Sardine"),ncol(lt.medians)],na.rm=TRUE)
  lt.max.sp.sar <- as.character(lt.median[lt.medians$median.var==lt.max.s,2])     # "Dominant sardine species"
  ...
  return(list(Dom_anch_LTmedian=as.character(lt.median.sp),
              Dom_sard_LTmedian=as.character(lt.median.sp.sar),
              dom.anch = dom.a.ts,dom.sard = dom.s.ts,
              max.table = as.data.frame(lt.medians)))
